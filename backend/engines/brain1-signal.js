...
class Brain1 {
  constructor({ dataService, newsService, db, strategyProfile = null }) {
  ...
  _topDownBias(mkt) {
    const weekly = this._bias(mkt.weekly, this.strategy.weeklySwings);
    const daily  = this._bias(mkt.daily,  this.strategy.dailySwings);
    const h4     = this._bias(mkt.h4,     this.strategy.h4Swings);
    const h2     = this._bias(mkt.h2,     this.strategy.h2Swings);

    if (this.strategy.allowDailyNeutralPullback) {
  ...
  _entryType(mkt, dir, loc) {
    ...
    const adxOk = h4.adx >= this.strategy.typeCAdxMin;
  ...
  _confluence(mkt, dir, loc, symbol, biasH2Conflicts) {
    ...
    const weeklyBias = this._bias(mkt.weekly, this.strategy.weeklySwings);
    const dailyBias  = this._bias(mkt.daily, this.strategy.dailySwings);
    f.htfAligned = weeklyBias === dir && (dailyBias === dir || (this.strategy.allowDailyNeutralPullback && dailyBias === 'NEUTRAL'));

    // Factor 2: Price at key level
    f.keyLevel = loc.valid;

    // Factor 3: Volume above 20-period average.
    // Forex volume feeds are often sparse or flat, so fall back to activity expansion.
    const avgVol = vol.length > 0 ? vol.slice(-20).reduce((a,b) => a+b, 0) / Math.min(vol.length, 20) : 0;
    const hasUsableVolume = vol.filter(v => v > 0).length >= Math.max(3, Math.floor(Math.min(vol.length, 20) / 2));
    f.volume = hasUsableVolume
      ? avgVol > 0 && vol[vol.length-1] > avgVol
      : this._activityExpansion(h4, 1.12, 0.45);
  ...
  _volumeExpansion(tf) {
    ...
    const usableVols = vols.filter(v => v > 0);
    if (usableVols.length < 3) return this._activityExpansion(tf, 1.25, 0.55);
  ...
  _activityExpansion(tf, rangeMultiplier = 1.1, minBodyRatio = 0.45) {
    const candles = tf?.candles?.slice(-8) || [];
    if (candles.length < 6) return false;
    const recent = candles[candles.length - 1];
    const recentRange = recent.high - recent.low;
    if (!isFinite(recentRange) || recentRange <= 0) return false;
    const prior = candles.slice(0, -1);
    const avgRange = prior.reduce((sum, candle) => sum + (candle.high - candle.low), 0) / prior.length;
    if (!isFinite(avgRange) || avgRange <= 0) return false;
    const bodyRatio = Math.abs(recent.close - recent.open) / recentRange;
    return recentRange >= avgRange * rangeMultiplier && bodyRatio >= minBodyRatio;
  }
